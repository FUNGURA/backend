...
export class RegisterClientDto {
    ...
    @ApiProperty({ description: 'Password for the account', minLength: 8, required: true })
    @IsString()
    @MinLength(8)
    @IsNotEmpty()
    password: string;

    @ApiProperty({ description: 'Gender of the client', enum: UGender, default: UGender.PREFER_NOT_TO_SAY, example: 'MALE' })
    @IsEnum(UGender)
    gender: UGender;
    ...
    @ApiProperty({ description: 'Date of birth of the client', required: false, example: '1990-01-01' })
    @IsOptional()
    dateOfBirth?: Date;
}